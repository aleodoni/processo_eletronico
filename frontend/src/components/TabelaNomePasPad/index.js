...
function TabelaNomePasPad({ proId }) {
    const [rows, setRows] = useState([]);

    const carregaMembrosComissao = useCallback(() => {
        axios({
            method: 'GET',
            url: `/dados-nome-pas-pad/${proId}`,
            headers: {
            ...
            .then(res => {
                ...
                for (let i = 0; i < res.data.length; i++) {
                    membros.push({
                        seq: i + 1,
                        matricula: res.data[i].nom_matricula,
                        nome: res.data[i].nom_nome,
                        login: res.data[i].nom_login,
                        area: res.data[i].nom_area_nome,
                    });
                    res.data.seq = i + 1;
                }
                setRows(membros);
    ...
    return (
        <TableContainer component={Paper}>
            <Table aria-label="table" size="small">
                ...
                <TableBody>
                    {rows.map(linha => (
                        <Row key={linha.nom_id} linha={linha} />
                    ))}
                </TableBody>
            </Table>
        </TableContainer>
...
export default memo(TabelaNomePasPad);

TabelaNomePasPad.propTypes = {
