'use strict';

const viewName = 'spa2.v_manifestacao_processo';

const sql = `
SELECT row_number() OVER (ORDER BY a.man_id) AS seq,
a.man_id,
a.pro_id,
b.tmn_nome,
a.man_login,
c.set_nome,
a.man_data
FROM spa2.manifestacao a,
spa2.tipo_manifestacao b,
spa2.v_area c,
spa2.processo d
WHERE ((a.tmn_id = b.tmn_id) AND (a.man_id_area = c.set_id) AND (a.pro_id = d.pro_id))
ORDER BY a.man_data;

`;

module.exports = {
    up: async(queryInterface, Sequelize) => {
        const transaction = await queryInterface.sequelize.transaction();
        try {
            await queryInterface.sequelize.query(`CREATE OR REPLACE VIEW ${viewName} AS ${sql}`);
            await transaction.commit();
            return Promise.resolve();
        } catch (err) {
            await transaction.rollback();
            return Promise.reject(err);
        }
    },

    down: async(queryInterface, Sequelize) => {
        await queryInterface.sequelize.query(`DROP VIEW ${viewName}`);
    }
};
