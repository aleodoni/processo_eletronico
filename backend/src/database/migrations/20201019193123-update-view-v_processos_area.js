...
SELECT a.pro_id,
    c.nod_id,
    spa2.verifica_observacao_tramite(a.pro_id, c.nod_id) AS ind_observacao,
    a.pro_codigo,
    b.tpr_nome,
    b.tpr_tramitacao_aberta,
   ...
   FROM spa2.processo a,
    ...
    spa2.nodo c
  WHERE ((a.tpr_id = b.tpr_id) AND (a.set_id_finalizador IS NULL) AND (a.usu_finalizador IS NULL) AND (a.nod_id = c.nod_id) AND (b.tpr_visualizacao <> 2))
  ORDER BY
        CASE
            WHEN (date_part('day'::text, (now() - (( SELECT t.tra_envio
               FROM spa2.tramite t
